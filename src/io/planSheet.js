...
function drawMarkerPins(be, L, floor) {
  ...
  for (const c of floor.constraints || []) {
    ...
    if (c.axis === 'x') {
      ...
      drawDimLabel(be, label, (xa + xb) / 2, y, C_PIN);
    } else {
      ...
      drawDimLabel(be, label, x, (ya + yb) / 2, C_PIN);
    }
  }
}
...
export function drawMarkerGlyph(be, cx, cy, type, size = 2.6) {
  ...
  } else { // outlet (default): socket circle + two contacts
    be.circle(cx, cy, r, { fill: '#fff', stroke: C_MARK, width: 0.2 });
    be.line(cx - r * 0.35, cy - r * 0.35, cx - r * 0.35, cy + r * 0.35, { stroke: C_MARK, width: 0.25 });
    be.line(cx + r * 0.35, cy - r * 0.35, cx + r * 0.35, cy + r * 0.35, { stroke: C_MARK, width: 0.25 });
  }
}
...
function drawMarkers(be, L, floor) {
  for (const m of floor.markers || []) {
    ...
    if (typeof m.z === 'number') {
      be.text(fmt(m.z), L.X(m.x), L.Y(m.y) + 3.4, { fill: C_MARK, size: 1.9, align: 'center', baseline: 'top' });
    }
  }
}
